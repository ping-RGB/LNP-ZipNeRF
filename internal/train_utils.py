...
def compute_data_loss(batch, renderings, config):
    ...
    if config.disable_multiscale_loss:#假
        lossmult = torch.ones_like(lossmult)

    for rendering in renderings:
        # resid_sq = (rendering['rgb'] - batch['rgb'][..., :3]) ** 2
        resid_sq1 = (rendering['rgb'] - batch['rgb'][..., :3]) ** 2
        resid_sq = (rendering['rgb'] - batch['rgb1'][..., :3]) ** 2
        denom = lossmult.sum()
        stats['mses'].append(((lossmult * resid_sq).sum() / denom).item())

        if config.data_loss_type == 'mse':#假
            # Mean-squared error (L2) loss.
            data_loss = resid_sq
        elif config.data_loss_type == 'charb':#真
            # Charbonnier loss.
            data_loss = torch.sqrt(0.2*resid_sq +0.8*resid_sq1+ config.charb_padding ** 2)
        elif config.data_loss_type == 'rawnerf':#假
            # Clip raw values against 1 to match sensor overexposure behavior.
            rgb_render_clip = rendering['rgb1'].clamp_max(1)
        ...
        if config.compute_disp_metrics:#假
        ...
        if config.compute_normal_metrics:#假
...
def anti_interlevel_loss(ray_history, config):
    ...
    for i, ray_results in enumerate(ray_history[:-1]):
        ...
        # test1 = w_s - wp
        # test2 = (w_s - wp).clamp_min(0) ** 2
        # test3 = wp + 1e-5
        # test4 = (w_s - wp).clamp_min(0) ** 2 / (wp + 1e-5)
...
def create_optimizer(config: configs.Config, model):
    ...
    lr_fn_main = lambda step: math.learning_rate_decay(
        ...
        **lr_kwargs)
    main_params = [param for name, param in model.named_parameters() if 'noise_sim' not in name]

    # noise_sim 的参数
    noise_sim_params = [param for name, param in model.named_parameters() if 'noise_sim' in name]

    # 创建优化器
    optimizer = torch.optim.Adam([
        {'params': main_params, 'lr': config.lr_init, 'name': 'main'},
        {'params': noise_sim_params, 'lr': config.noise_sim_lr_init, 'name': 'noise_sim'}
    ], **adam_kwargs)
