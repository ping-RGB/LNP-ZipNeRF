...
class Dataset(torch.utils.data.Dataset):
  ...
  """
    ...
    """
        ...
        batch['rgb1'] = self.images_pre[cam_idx, pix_y_int, pix_x_int]
        if self._load_disps:
...
class LLFF(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        if config.rawnerf_mode:
            ...
            # Load images.
            colmap_image_dir = os.path.join(self.data_dir, 'images_2')
            ...
            ################################################################################################################################
            colmap_image_dir1 = os.path.join(self.data_dir, 'images_2')
            image_dir1 = os.path.join(self.data_dir, 'images_test')
            for d in [image_dir1, colmap_image_dir1]:
                if not utils.file_exists(d):
                    raise ValueError(f'Image folder {d} does not exist.')
            # Downsampled images may have different names vs images used for COLMAP,
            # so we need to map between the two sorted lists of files.
            colmap_files1 = sorted(utils.listdir(colmap_image_dir1))
            image_files1 = sorted(utils.listdir(image_dir1))
            colmap_to_image1 = dict(zip(colmap_files1, image_files1))
            image_paths = [os.path.join(image_dir1, colmap_to_image1[f])
                           for f in image_names]
            images_4 = [utils.load_img(x) for x in
                      tqdm(image_paths, desc='Loading LLFF dataset', disable=self.global_rank != 0, leave=False)]
            images_4 = np.stack(images_4, axis=0) / 255.

            colmap_image_dir1 = os.path.join(self.data_dir, 'images_2')
            image_dir_pre = os.path.join(self.data_dir, 'images_pre')
            for d in [image_dir_pre, colmap_image_dir1]:
                if not utils.file_exists(d):
                    raise ValueError(f'Image folder {d} does not exist.')
            # Downsampled images may have different names vs images used for COLMAP,
            # so we need to map between the two sorted lists of files.
            colmap_files = sorted(utils.listdir(colmap_image_dir1))
            image_files = sorted(utils.listdir(image_dir_pre))
            colmap_to_image = dict(zip(colmap_files, image_files))
            # print("Keys in colmap_to_image:", colmap_to_image.keys())
            # print("File name causing error:", 'DSCF5806.JPG')

            image_paths = [os.path.join(image_dir_pre, colmap_to_image[f])
                           for f in image_names]
            images_pre = [utils.load_img(x) for x in
                          tqdm(image_paths, desc='Loading LLFF dataset', disable=self.global_rank != 0, leave=False)]
            images_pre = np.stack(images_pre, axis=0) / 255.
        ...
        self.height, self.width = images_4.shape[1:3]
        self.images_pre = images_pre
        self.height, self.width = images_pre.shape[1:3]
